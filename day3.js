...
let step1 = 0;
for (i1 of input31) {
	end3 = parseInt(i1.substring(1, i1.length));
	switch (i1[0]) {
		case 'R':
			for (j = 0; j < end3; j++) {
				pointIdx[1] = pointIdx[1] + 1;
				step1 = step1 + 1;
				pointMatrix1.push([pointIdx[0], pointIdx[1], step1]);
			}
			break;
		case 'D':
			for (j = 0; j < end3; j++) {
				pointIdx[0] = pointIdx[0] + 1;
				step1 = step1 + 1;
				pointMatrix1.push([pointIdx[0], pointIdx[1], step1]);
			}
			break;
	...
	switch (i1[0]) {
		case 'R':
			for (j = 0; j < end3; j++) {
				pointIdx[1] = pointIdx[1] + 1;
				step2 = step2 + 1;
				pointMatrix2.push([pointIdx[0], pointIdx[1], step2]);
			}
			break;
		case 'D':
			for (j = 0; j < end3; j++) {
...
const inters3 = []
const idxs3 = []
for (p1 of pointMatrix1) {
	for (p2 of pointMatrix2) {
		if (p1[0] === p2[0] && p1[1] === p2[1]) {
			inters3.push(p1);
			idxs3.push(p1[2] + p2[2])
		}
	}
}
...
let mins3;
let minidx3;
console.log("Intersection done");

for (i3 of inters3) {
	const d3 = Math.abs(i3[0]) + Math.abs(i3[1]);
	if (!mins3 || d3 < mins3) {
		mins3 = d3;
	}
}
